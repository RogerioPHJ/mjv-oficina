...
$(document).ready(
	function() {
		...
		mdc.textField.MDCTextField.attachTo(document
			.querySelector('.mdc-text-field'));
		mdc.form
			- field.MDCFormField.attachTo(document
				.querySelector('.mdc-form-field'));
		mdc.checkbox.MDCCheckbox.attachTo(document
			.querySelector('.mdc-checkbox'));
	})

let checked = 0

$(".mdc-checkbox__native-control").change(
	function() {
		if (this.checked) {
			checked++;
		} else {
